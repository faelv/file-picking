...
uses
  Vcl.Forms, System.Types, Winapi.Windows, System.SysUtils, System.Win.ComObj,
...
function SetTaskbarOverlayIcon(const AIcon: THandle; const ADescription: String)
  : Boolean; overload;
function SetTaskbarOverlayIcon(const AIcon: TIcon; const ADescription: String)
  : Boolean; overload;
function SetTaskbarOverlayIcon(const AList: TImageList; const IconIndex: Integer;
  const ADescription: String): Boolean; overload;
...
type
  ITaskBarList3 = interface(IUnknown)
    ...
    function SetProgressValue(hwnd: THandle; ullCompleted: UInt64; ullTotal: UInt64)
      : HRESULT; stdcall;
    ...
    function SetTabActive(hwndTab: THandle; hwndMDI: THandle; tbatFlags: Cardinal)
      : HRESULT; stdcall;
    function ThumbBarAddButtons(hwnd: THandle; cButtons: Cardinal; pButtons: Pointer)
      : HRESULT; stdcall;
    function ThumbBarUpdateButtons(hwnd: THandle; cButtons: Cardinal; pButtons: Pointer)
      : HRESULT; stdcall;
    function ThumbBarSetImageList(hwnd: THandle; himl: THandle): HRESULT; stdcall;
    function SetOverlayIcon(hwnd: THandle; hIcon: THandle; pszDescription: PChar)
      : HRESULT; stdcall;
...
begin
  ...
  if CheckAPI then begin
    ...
    Result := GlobalTaskBarInterface.SetProgressState(Application.MainForm.Handle, Flag) = S_OK;
  end;
end;
...
begin
  ...
  if CheckAPI then begin
    Result := GlobalTaskBarInterface.SetProgressValue(Application.MainForm.Handle, ACurrent,
      AMax) = S_OK;
  end;
end;
...
begin
  ...
  if CheckAPI then begin
    Result := GlobalTaskBarInterface.SetOverlayIcon(Application.MainForm.Handle, AIcon,
      PChar(ADescription)) = S_OK;
  end;
end;
